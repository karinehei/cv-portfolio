...
2. Open [GitHub Actions](https://github.com/karinehei/cv-portfolio/actions) — CI: lint, build, Newman (7 API tests), Robot UI (16 tests)  
...
**CI workflow:** [`.github/workflows/qa-tests.yml`](.github/workflows/qa-tests.yml) (lint, build, Newman, Robot on every push/PR)
...
| **Regression** | Newman API and Robot UI suites on each CI run |
...
Newman API and Robot UI tests run in [`.github/workflows/qa-tests.yml`](.github/workflows/qa-tests.yml) on every push. CI uses system Chrome (no Playwright Chromium download). Locally, run `rfbrowser init chromium` once — see [`tests/robot/README.md`](tests/robot/README.md).
